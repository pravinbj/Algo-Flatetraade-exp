...
def fetch_option_data():
    ...
    UNDERLYING_CONFIG = [
        {'name': 'NIFTY', 'index_token': '26000', 'strike_step': Config.NIFTY_STRIKE_STEP, 
         'expiry': Config.NIFTY_EXPIRY, 'strikes_count': Config.NIFTY_STRIKES_COUNT},
        {'name': 'Nifty Bank', 'index_token': '26009', 'strike_step': Config.BANKNIFTY_STRIKE_STEP, 
         'expiry': Config.BANKNIFTY_EXPIRY, 'strikes_count': Config.BANKNIFTY_STRIKES_COUNT}
    ]
    
    start_datetime = Config.FETCH_DATE + ' 09:15:00'; end_datetime = Config.FETCH_DATE + ' 15:30:00'
    start_timestamp = get_time(start_datetime); end_timestamp = get_time(end_datetime)
    
    print(f"\n{'='*70}"); print(f"FETCHING OPTION DATA FOR {Config.FETCH_DATE}"); print(f"{'='*70}")
    print(f"Time Range: {start_datetime} to {end_datetime}")
    print(f"NIFTY Expiry: {Config.NIFTY_EXPIRY} | Strikes: ±{Config.NIFTY_STRIKES_COUNT}")
    print(f"BANKNIFTY Expiry: {Config.BANKNIFTY_EXPIRY} | Strikes: ±{Config.BANKNIFTY_STRIKES_COUNT}")
    print(f"Time Interval: {Config.TIME_INTERVAL} minutes"); print(f"{'='*70}")
    success_count = 0
    
    for config in UNDERLYING_CONFIG:
        underlying = config['name']; index_token = config['index_token']; strike_step = config['strike_step']
        expiry_str = config['expiry']; strikes_count = config['strikes_count']
        ...
        ltp = float(idx_q.get('lp', 0)); atm = int(round(ltp / strike_step) * strike_step)
        ...
        # Generate strikes based on strikes_count from config
        strikes = []
        ...
        for strike in strikes:
            for typ in ['C', 'P']:
                symbol = f"{base}{expiry_str}{typ}{strike}"; print(f"  Fetching: {symbol}")
                token, found_symbol = search_specific_contract(api, symbol)
                if not token: print(f"    ⚠ Contract not found"); continue
                ...
                if not ret or (isinstance(ret, dict) and ret.get('stat') == 'Not_Ok'): print(f"    ⚠ No data returned"); continue
                try: df = pd.DataFrame(ret)
                except Exception as e: print(f"    ⚠ DataFrame error: {e}"); continue
                if df.empty: print(f"    ⚠ Empty dataframe"); continue
                ...
                filename = f"{symbol}.csv"; outfile = os.path.join(Config.DATA_DIR, filename); df.to_csv(outfile, index=False)
                print(f"    ✓ Saved: {filename}"); success_count += 1; time.sleep(0.5)
    
    print(f"\n{'='*70}"); print(f"Data fetch completed!"); print(f"Files saved: {success_count}")
    print(f"Directory: {Config.DATA_DIR}"); print(f"Time Interval: {Config.TIME_INTERVAL} minutes")
    print(f"{'='*70}")
